    ...
    if "pdf" in request.files:
        try:
            ...
            try:
                ...
                padrao = r'(?i)(?:cap[ií]tulo\s+[\dIVXLCDMivxlcdm]+|cap\.\s*\d+)'
                partes = re.split(padrao, texto_total)
                titulos_cap = re.findall(padrao, texto_total)

                if len(partes) > 2 and len(titulos_cap) >= 2:
                    for i, conteudo in enumerate(partes[1:], 1):
                        titulo_cap = titulos_cap[i-1] if i-1 < len(titulos_cap) else f"Capítulo {i}"
                        pars = [p.strip() for p in conteudo.split('\n') if p.strip() and len(p.strip()) > 20]
                        html = ''.join(f'<p>{p}</p>' for p in pars[:80])
                        if html:
                            capitulos.append({"numero": i, "titulo": titulo_cap.strip(), "conteudo": html})
                else:
                    # Dividir por blocos de texto
                    linhas = [l.strip() for l in texto_total.split('\n') if l.strip() and len(l.strip()) > 15]
                    bloco = max(30, len(linhas) // 20)
                    for i, start in enumerate(range(0, len(linhas), bloco), 1):
                        pars = linhas[start:start+bloco]
                        html = ''.join(f'<p>{p}</p>' for p in pars)
                        if html:
                            capitulos.append({"numero": i, "titulo": f"Capítulo {i}", "conteudo": html})

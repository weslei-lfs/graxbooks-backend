...
def criar_livro():
    ...
    # Upload PDF + extrair texto
    if "pdf" in request.files:
        pdf_file = request.files["pdf"]
        pdf_bytes = pdf_file.read()

        # Subir para Cloudinary
        up = cloudinary.uploader.upload(
            io.BytesIO(pdf_bytes),
            folder="graxbooks/pdfs",
            resource_type="raw",
            public_id=f"{titulo.replace(' ', '_')}_{datetime.utcnow().strftime('%Y%m%d%H%M%S')}",
        )
        ...
        try:
            ...
            # Dividir em capítulos por padrões comuns
            # Tenta detectar capítulos pelo padrão "Capítulo X", "CAPÍTULO X", "Cap. X"
            padrao = r'(?i)(?:cap[ií]tulo\s+[\dIVXLCDM]+|cap\.\s*\d+|chapter\s+\d+)'
            partes = re.split(padrao, texto_total)
            titulos_cap = re.findall(padrao, texto_total)

            if len(partes) > 1:
                # PDF tem marcadores de capítulo
                for i, conteudo in enumerate(partes[1:], 1):
                    titulo_cap = titulos_cap[i-1] if i-1 < len(titulos_cap) else f"Capítulo {i}"
                    texto_limpo = conteudo.strip()
                    if len(texto_limpo) > 50:
                        # Formatar como HTML
                        paragrafos = [p.strip() for p in texto_limpo.split('\n\n') if p.strip() and len(p.strip()) > 20]
                        html = ''.join(f'<p>{p.replace(chr(10), " ")}</p>' for p in paragrafos[:50])
                        if html:
                            capitulos.append({"numero": i, "titulo": titulo_cap.strip(), "conteudo": html})
            else:
                # Sem capítulos detectados — dividir por páginas (grupos de texto)
                paragrafos_todos = [p.strip() for p in texto_total.split('\n\n') if p.strip() and len(p.strip()) > 30]
                # Agrupar em blocos de ~20 parágrafos por capítulo
                tam_bloco = max(20, len(paragrafos_todos) // max(1, paginas // 10))
                num_cap = 1
                for i in range(0, len(paragrafos_todos), tam_bloco):
                    bloco = paragrafos_todos[i:i+tam_bloco]
                    html = ''.join(f'<p>{p.replace(chr(10), " ")}</p>' for p in bloco)
                    if html:
                        capitulos.append({"numero": num_cap, "titulo": f"Capítulo {num_cap}", "conteudo": html})
                        num_cap += 1

        except ImportError:
            # PyMuPDF não instalado — capítulo placeholder
            capitulos = [{"numero": 1, "titulo": "Conteúdo", "conteudo": "<p>Instale PyMuPDF para extração automática de texto.</p>"}]
        except Exception as e:
            capitulos = [{"numero": 1, "titulo": "Conteúdo", "conteudo": f"<p>Erro ao extrair texto: {str(e)}</p>"}]
    ...
    if capitulos:
        caps_insert = [
            {"livro_id": livro_id, "numero": c["numero"], "titulo": c["titulo"], "conteudo": c["conteudo"]}
            for c in capitulos
        ]
        # Inserir em lotes de 50
        for i in range(0, len(caps_insert), 50):
            supabase.table("capitulos").insert(caps_insert[i:i+50]).execute()

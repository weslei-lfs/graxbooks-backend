...
def criar_livro():
    ...
    if "pdf" in request.files:
        try:
            ...
            # Extrair texto com PyMuPDF — página por página
            try:
                import fitz
                doc = fitz.open(stream=pdf_bytes, filetype="pdf")
                paginas = len(doc)

                # Extrair cada página individualmente
                paginas_texto = []
                for i, page in enumerate(doc):
                    txt = page.get_text().strip()
                    if txt and len(txt) > 30:  # só páginas com conteúdo real
                        paginas_texto.append(txt)
                doc.close()

                # Agrupar páginas em capítulos de leitura confortável
                # Cada "capítulo" = grupo de 8 páginas do PDF
                PAGINAS_POR_CAP = 8
                num_cap = 1
                for i in range(0, len(paginas_texto), PAGINAS_POR_CAP):
                    grupo = paginas_texto[i:i+PAGINAS_POR_CAP]
                    # Juntar páginas do grupo e formatar em parágrafos
                    html_partes = []
                    for pg_txt in grupo:
                        linhas = [l.strip() for l in pg_txt.split('\n') if l.strip() and len(l.strip()) > 10]
                        # Agrupar linhas em parágrafos
                        paragrafo = []
                        for linha in linhas:
                            paragrafo.append(linha)
                            # Nova parágrafo quando linha termina com ponto ou é longa
                            if linha.endswith('.') or linha.endswith('!') or linha.endswith('?') or len(linha) > 80:
                                html_partes.append(f'<p>{" ".join(paragrafo)}</p>')
                                paragrafo = []
                        if paragrafo:
                            html_partes.append(f'<p>{" ".join(paragrafo)}</p>')

                    html = ''.join(html_partes)
                    if html:
                        pag_inicio = i + 1
                        pag_fim = min(i + PAGINAS_POR_CAP, len(paginas_texto))
                        capitulos.append({
                            "numero": num_cap,
                            "titulo": f"Páginas {pag_inicio}–{pag_fim}",
                            "conteudo": html
                        })
                        num_cap += 1

            except ImportError:
                capitulos = [{"numero":1,"titulo":"Conteúdo","conteudo":"<p>PDF enviado. Extração de texto requer PyMuPDF.</p>"}]
            except Exception as e:
                capitulos = [{"numero":1,"titulo":"Conteúdo","conteudo":f"<p>Erro na extração: {str(e)[:200]}</p>"}]
        except Exception as e:
            print(f"Erro PDF: {e}")
